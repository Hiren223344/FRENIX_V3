...
v1Router.post('/chat/completions', rateLimitMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (requestPayload.stream) {
      ...
      const generator = streamChatCompletion(requestPayload, user.apiKey);
    ...
    const result = await createChatCompletion(requestPayload, user.apiKey);
...
const handleAnthropicMessages = async (req: Request, res: Response) => {
  try {
    ...
    if (requestPayload.stream) {
      ...
      const generator = streamAnthropicMessage(requestPayload, user.apiKey, anthropicVersion);
    ...
    const result = await createAnthropicMessage(requestPayload, user.apiKey, anthropicVersion);
